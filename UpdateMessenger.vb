...
Module UpdateMessenger
    Sub ShowMessage()
        Try
            ...
            UpdMsg = $"Возвращена возможность сохранять таблицу проверки стандарта. Обратите внимания, сохранять можно только полную таблицу. Для этого необходимо снять отметку о показе элементов, у которых есть расчетные значения превыщающие паспортные на заданное число. Добавлен английский интерфейс{vbCrLf}Добавлены следующие изменения в отображении файлов и таблиц концентраций:{vbCrLf}{vbTab}1. Изменен формат файлов концентраций, однако, программа совместима со старым форматом .con{vbCrLf}{vbTab}2. Появилась возможность просматривать значения активностей элементов, паспортные значения которых не найдены в эталоне.{vbCrLf}{vbTab}3. Отмеченная опция 'Рассчитать для фильтров' выводит значения концентраций в микрограммах.{vbCrLf}{vbTab}4. Изменено поведение работы редактора ГРС при проверке стандартов. Теперь после автоматического создания ГРС нельзя его проверить. В самой проверке, по умолчанию показываются только те элементы, у которых одно из рассчитанных значений превышает заданный предел.(По умолчанию 30%).{vbCrLf}{vbTab}5. Повышена стабильность приложения.{vbCrLf}{vbTab}6. Исправлены мелкие ошибки."
            'update message
