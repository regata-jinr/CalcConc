...
Module UpdateMessenger
    Sub ShowMessage()
        Try
            ...
            UpdMsg = $"{vbTab}Добавлен редактор просмотра значений концентраций с возможностью считать средние значения. Для использования необходимо, как обычно, выбрать файлы концентраций(для построения таблиц). Затем нажать на появившуюся кнопку просмотр значений концентраций. В открытом окне отобразиться содержимое выбранных файлов, а также усредненные значения. На мой взгляд, интерфейс окна примитивный и в описании не нуждается.{vbCrLf}{vbCrLf}{vbTab}Исправлены некоторые неточности{vbCrLf}"
            'update message
